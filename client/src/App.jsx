  ...
  (
    ...
    useImperativeHandle(ref, () => ({
      ...
      undo: () => {
        ...
        const lastStrokeId =
          strokesHistory.current[strokesHistory.current.length - 1].strokeId;
        strokesHistory.current = strokesHistory.current.filter(
          (s) => s.strokeId !== lastStrokeId,
        );
      ...
      remoteUndo: (strokeId) => {
        if (strokeId) {
          ...
          const lastStrokeId =
            strokesHistory.current.length > 0
              ? strokesHistory.current[strokesHistory.current.length - 1]
                  .strokeId
              : null;
          strokesHistory.current = strokesHistory.current.filter(
            (s) => s.strokeId !== lastStrokeId,
      ...
      playReplay: () => {
        return new Promise((resolve) => {
          ...
          const strokesPerFrame = Math.max(
            1,
            Math.ceil(strokes.length / framesTarget),
          );
          let currentIndex = 0;

          const drawFrame = () => {
            const endIndex = Math.min(
              currentIndex + strokesPerFrame,
              strokes.length,
            );
    ...
    const startDrawing = (e) => {
      ...
      // Immediately draw a dot for single clicks
      if (!isLineTool) {
        const strokeObj = {
          strokeId: currentStrokeId.current,
          startX: x,
          startY: y,
          endX: x + 0.01, // Adding 0.01 ensures older browsers correctly render the dot
          endY: y,
          color,
          size: brushSize,
        };
    ...
    const endDrawing = (e) => {
      ...
      if (
        isDrawing &&
        isLineTool &&
        lineStartPos.current &&
        previewEndPos.current
      ) {
        const strokeObj = {
          strokeId: currentStrokeId.current,
    ...
    const getCursor = () => {
    if (!isPlayer) return 'default';
    if (isLineTool) return 'crosshair';
    
    const size = Math.max(brushSize, 1);
    // Ensure the SVG canvas is always large enough to hold the precision crosshair (at least 20px)
    const svgSize = Math.max(size + 4, 20); 
    const center = svgSize / 2;
    const r = Math.max((size / 2) - 1, 0.5);

    // Always show a precise aiming crosshair in the center
    let crosshairSvg = `
      <line x1="${center - 4}" y1="${center}" x2="${center + 4}" y2="${center}" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
      <line x1="${center}" y1="${center - 4}" x2="${center}" y2="${center + 4}" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
      <line x1="${center - 3}" y1="${center}" x2="${center + 3}" y2="${center}" stroke="black" stroke-width="1" stroke-linecap="round"/>
      <line x1="${center}" y1="${center - 3}" x2="${center}" y2="${center + 3}" stroke="black" stroke-width="1" stroke-linecap="round"/>
    `;

    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}">
        ${crosshairSvg}
        <circle cx="${center}" cy="${center}" r="${r}" fill="none" stroke="white" stroke-width="1.5"/>
        <circle cx="${center}" cy="${center}" r="${r}" fill="none" stroke="black" stroke-width="0.5"/>
      </svg>
    `;
    const b64 = btoa(svg);
    const dataUri = `data:image/svg+xml;base64,${b64}`;
    return `url(${dataUri}) ${center} ${center}, crosshair`;
  };
...
function App() {
  ...
  const [timeLeft, setTimeLeft] = useState(
    parseInt(import.meta.env.VITE_MATCH_DURATION_SECONDS || 180),
  );
  ...
  useEffect(() => {
    const handleFirstInteraction = () => {
      if (!hasAutoPlayedBgm.current && !isMuted) {
        hasAutoPlayedBgm.current = true;
        toggleBGM(true);
      }
  ...
  const handleCanvasScroll = (e) => {
    if (refScrollRef.current) {
      if (refScrollRef.current.scrollTop !== e.target.scrollTop)
        refScrollRef.current.scrollTop = e.target.scrollTop;
      if (refScrollRef.current.scrollLeft !== e.target.scrollLeft)
        refScrollRef.current.scrollLeft = e.target.scrollLeft;
    }
  };

  const handleRefScroll = (e) => {
    if (canvasScrollRef.current) {
      if (canvasScrollRef.current.scrollTop !== e.target.scrollTop)
        canvasScrollRef.current.scrollTop = e.target.scrollTop;
      if (canvasScrollRef.current.scrollLeft !== e.target.scrollLeft)
        canvasScrollRef.current.scrollLeft = e.target.scrollLeft;
    }
  };
  ...
  useEffect(() => {
    ...
    newSocket.on(
      'initialState',
      ({ strokes, scores, referenceImage, gameStarted, endTime }) => {
        if (referenceImage) {
    ...
    newSocket.on('gameStarted', ({ endTime }) => {
      ...
      setTimeout(() => setShowDrawAnimation(false), 1500);
    });

    newSocket.on('gameOver', ({ scores }) => {
      setGameStatus('finished');
      if (scores) {
        if (scores[newSocket.id] !== undefined)
          setMyScore(scores[newSocket.id]);
        const oppId = Object.keys(scores).find((id) => id !== newSocket.id);
        if (oppId && scores[oppId] !== undefined)
          setOpponentScore(scores[oppId]);
      }
      ...
      setTimeout(() => {
        if (referenceCanvasRef.current) {
          if (myCanvasRef.current) {
            const myDiff = generateDiffOverlay(
              myCanvasRef.current.getCanvas(),
              referenceCanvasRef.current,
            );
            if (myDiff) setMyDiffUrl(myDiff);
          }
          if (oppCanvasRef.current) {
            const oppDiff = generateDiffOverlay(
              oppCanvasRef.current.getCanvas(),
              referenceCanvasRef.current,
            );
    ...
    newSocket.on('opponentWantsRematch', () =>
      setRematchStatus('opponent_waiting'),
    );
  ...
  const createGame = async () => {
    ...
    if (gameCategory === 'custom') {
      ...
      const dicebearBase =
        import.meta.env.VITE_DICEBEAR_API_URL || 'https://api.dicebear.com/7.x';
  ...
  // Score Counting Animation & Confetti
  useEffect(() => {
    if (gameStatus === 'finished') {
      let currentMy = 0;
      let currentOpp = 0;
      ...
      const myStep = myScore / steps;
      const oppStep = opponentScore / steps;
      ...
      const timer = setInterval(() => {
        ...
        if (currentMy >= myScore) currentMy = myScore;
        if (currentOpp >= opponentScore) currentOpp = opponentScore;

        setDisplayMyScore(Math.floor(currentMy));
        setDisplayOppScore(Math.floor(currentOpp));

        if (tickCount % 2 === 0) playScoreTick(1 + currentMy / 100); // Pitch goes up

        if (currentMy === myScore && currentOpp === opponentScore) {
          clearInterval(timer);
          if (myScore > opponentScore && myScore >= 50) {
            triggerShake();
            confetti({
              particleCount: 150,
              spread: 70,
              origin: { y: 0.6 },
              colors: ['#f43f5e', '#10b981', '#3b82f6', '#fbbf24'],
            });
          }
        }
      }, 1000 / fps);
      ...
      setDisplayMyScore(0);
      setDisplayOppScore(0);
    }
  }, [gameStatus, myScore, opponentScore]);
  ...
  const handleImageError = () => {
    ...
    console.warn(
      'Failed to load AI image. Falling back to deterministic default based on room ID.',
    );
    let hash = 0;
  ...
  const updateMyScore = (score) => {
    setMyScore((prev) => {
      if (gameStatus === 'playing' && score - prev >= 3) {
        const text =
        ...
        playCombo();
        setTimeout(() => {
          setComboPopups((curr) => curr.filter((p) => p.id !== id));
        }, 1500);
      }
      return score;
    });
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{ padding: '40px', textAlign: 'center', minWidth: '350px' }}
        >
          <div
            style={{
              display: 'flex',
          ...
          >
            ...
            >
              ...
              >
                <option value='shapes' style={{ color: 'black' }}>
                  Abstract Shapes
                </option>
                <option value='pixel-art' style={{ color: 'black' }}>
                  Pixel Art
                </option>
                <option value='icons' style={{ color: 'black' }}>
                  Icons (Beginner)
                </option>
                <option value='fun-emoji' style={{ color: 'black' }}>
                  Fun Emoji
                </option>
                <option value='avataaars' style={{ color: 'black' }}>
                  Avatars
                </option>
                <option value='bottts' style={{ color: 'black' }}>
                  Robots
                </option>
                <option value='adventurer' style={{ color: 'black' }}>
                  Adventurer
                </option>
                <option value='croodles' style={{ color: 'black' }}>
                  Croodles
                </option>
                <option value='identicon' style={{ color: 'black' }}>
                  Identicons
                </option>
                <option value='rings' style={{ color: 'black' }}>
                  Rings & Geometry
                </option>
                <option value='micah' style={{ color: 'black' }}>
                  Micah
                </option>
                <option value='miniavs' style={{ color: 'black' }}>
                  Mini Avatars
                </option>
                <option value='open-peeps' style={{ color: 'black' }}>
                  Open Peeps
                </option>
                <option value='personas' style={{ color: 'black' }}>
                  Personas
                </option>
                <option value='lorelei' style={{ color: 'black' }}>
                  Lorelei (Advanced)
                </option>
  ...
  return (
    <div className={`app-container ${isShaking ? 'shake' : ''}`}>
      ...
      >
        {gameStatus === 'finished' ? (
          ...
          >
            ...
            >
              {opponentLeft ? 'Opponent Forfeited' : "Time's Up"}
            </h2>
                    ...
                    style={{
                      ...
                      background:
                        rematchStatus === 'waiting'
                          ...
                          : 'var(--success)',
                      cursor:
                        rematchStatus === 'waiting' ? 'not-allowed' : 'pointer',
                    }}
                  >
                    {rematchStatus === 'waiting'
          ...
          <>
            ...
            <div
              className={
                timeLeft <= 10 && gameStatus === 'playing' ? 'timer-pulse' : ''
              }
              style={{
                fontSize: '2rem',
                fontWeight: 'bold',
                color: timeLeft <= 10 ? 'var(--danger)' : 'white',
              }}
            >
            ...
            <button
              ...
              style={{
                background: showGrid ? 'var(--accent)' : 'var(--panel-bg)',
              }}
            >
      ...
      {showDrawAnimation && <div className='draw-flash-text'>DRAW!</div>}
      ...
      <div
        ...
        style={{
          position: 'relative',
          flexWrap: gameStatus === 'finished' ? 'nowrap' : 'wrap',
          transform: gameStatus === 'finished' ? 'scale(0.85)' : 'none',
          transition: 'all 0.5s ease',
          marginTop: gameStatus === 'finished' ? '-20px' : '0',
        }}
      >
        {isImageLoading && (
          <div
        ...
        <div className='canvas-container'>
          {gameStatus === 'finished' ? (
            ...
            <>
              ...
              >
                <span>You</span>
                <span>
                  {gameStatus === 'finished' ? displayMyScore : myScore}% Match
                </span>
              </div>
          ...
          >
            <div
              className='responsive-canvas-wrapper'
              style={{
                width: `${displayZoom * 100}%`,
                maxWidth: 'none',
            ...
            >
              <Canvas
                ...
                color={
                  isEraser ? 'rgba(255,255,255,1)' : hexToRgba(color, opacity)
                }
                brushSize={brushSize}
                socket={socket}
                roomId={roomId}
              ...
              {comboPopups.map((popup) => (
                <div key={popup.id} className='combo-popup'>
                  {popup.text}
                </div>
              ))}
        ...
        >
          ...
          {showGrid && !isImageLoading && gameStatus !== 'finished' && (
          ...
          {gameStatus === 'finished' ? (
            ...
            <>
              ...
              >
                <span>Opponent</span>
                <span>
                  {gameStatus === 'finished' ? displayOppScore : opponentScore}%
                  Match
                </span>
              </div>
              <div className='accuracy-bar-container'>
                <div
                  className='accuracy-bar'
                  style={{
                    width: `${gameStatus === 'finished' ? displayOppScore : opponentScore}%`,
                  }}
                ></div>
              </div>
            </>
